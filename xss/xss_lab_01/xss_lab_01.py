...
def xss_test(url, use_proxy=False):
    try:
        ...
        search_url = urljoin(url, f'search?q={payload}')
        
        print(f"Sending XSS payload: {payload}")
        response = requests.get(search_url, proxies=proxies, verify=False, timeout=30)
        
        if payload in response.text:
            print("XSS payload successfully reflected in the response")
            return is_solved(url, proxies)
        else:
            print("XSS payload was not reflected in the response")
...
def main():
    parser = argparse.ArgumentParser(description='XSS Test for Lab 01')
